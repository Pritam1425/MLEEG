function c = Fast_kurtogram(x,nlevel,Fs,filename,num)
% Fast_Kurtogram(x,nlevel,Fs,filename)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    fi = (J-1)/3/2^(nlevel+1);   fi = fi + (double(2))^(double(-2-Level_w(I)));
    title(['fb-kurt.2 - K_{max}=',num2str(round(10*M)/10),' @ level ',num2str(fix(10*Level_w(I))/10),', Bw= ',num2str(Fs*2^(double(-(Level_w(I)+1)))),'Hz, f_c=',num2str(Fs*fi),'Hz'])
    hFig = gcf;
    hAx  = gca;
    ...
    saveas(gcf,fullfile(filename,num), 'png')
    close(gcf)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % SIGNAL FILTERING
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
